...
// Editable placeholder copy. Replace only these strings when approved glove
// and shoe descriptions are available.
const portraitCopy = {
  gloves: {
    title: "장갑",
...
// frames. Keep these bounds close to the visible glove and shoes.
...
export function validateCharacterConfig(config: CharacterConfig): void {
  ...
  if (config.hotspots.map(({ id }) => id).join(",") !== "gloves,shoes") {
    throw new Error("Character must define glove and shoe hotspots");
  }
}
